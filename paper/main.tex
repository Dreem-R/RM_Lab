...
% T1 fonts are used to generate the final print and online PDFs (Springer requirement).
\usepackage[utf8]{inputenc}

\usepackage{amsmath}
\usepackage{newtxtext}          % Times Roman text, per the LNCS template
\usepackage[varvw]{newtxmath}   % matching maths
...
\input{tables/tab_extraction_wrapper}
...
\input{tables/tab_missing_wrapper}
...
\input{tables/tab_cohort_wrapper}

\section{Results}

\subsection{RQ1: model comparison}

\input{tables/tab_results_wrapper}
...
\caption{(a) Decision curves over the threshold range that is plausible for an outcome
with 11\% prevalence. All four models are mutually indistinguishable and beat both
treat-all and treat-none up to $p_t \approx 0.11$; beyond that their net benefit falls
below treat-none, because the class-weighted probabilities plotted here are inflated
(Section~\ref{sec:imbalance}) rather than because the ranking fails.
...
\input{tables/tab_split_wrapper}
...
\input{tables/tab_imbalance_wrapper}
...
\input{tables/tab_ablation_wrapper}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.92\textwidth]{fig_shap.pdf}
...
\input{tables/tab_predictors_wrapper}
...
% flush any pending floats so no table or figure lands inside the reference list
\clearpage
